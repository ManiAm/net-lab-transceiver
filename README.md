...
- **[The Pluggable Transceiver Model](docs/01_README_module.md):** The three-layer pluggable architecture (port, module, cable), form factor families (SFP through OSFP), the QSFP28 electrical interface, cabling options (DAC, AOC, structured fiber), and reach standards.
- **[Optical Integration Architectures](docs/04_README_optics_integration.md):** The evolution from DSP-heavy pluggables to linear and co-packaged designs — retiming strategies (FRO, TRO, LPO), physical proximity form factors (Faceplate, XPO, NPO, CPO), and the tradeoffs driving the copper-to-optics transition in AI-scale networks.
